...
crash <- read_csv("~/GitHub/PPOL105/Traffic_Crashes_Orig.csv")
...
```{r}
crash <- crash %>%  # READ IN ORIGINAL FILE
...
#crash$CRASH_DATE <- mdy_hms(crash$CRASH_DATE) # Remove hours, I did this in excel already for the cleaned version. It also exists already in another column
...
count %>% 
  ...
  scale_x_continuous(name = "Number of Crashes")
```
```{r}
#crashes at each speed limit
speedcount <- crash %>% 
  group_by(speed_limit=POSTED_SPEED_LIMIT) %>% 
  summarize(n=n()) 

speedcount # as a tibble
```
...
speedcount %>% 
  kbl(caption = "Crashes in Each Speed Limit within Chicago") %>% 
...
kbl(speedcount) %>%
  kable_classic() %>%
  add_header_above(c(" " = 1, "Group 1" = 1)) # number of items in each group and gives them a label

# cooler if you are trying to grouop multiple variables in a chart. 
```
