...
**Objects** are repositories of information - numbers, text, images... Each object has a name and we can assign content to an object using **<-**. You can also use **=** but the arrow is generally preferred
...
An object that stores multiple pieces of information is called **vector**.
...
To inspect an object, you can just type its name.
...
- R is case sensitive: I suggest using lowercase as much as possible
...
**Strings** (or characters) are pieces of text information which are stored into **""** 
...
*Which is the class of each vector?*
...
*What is the different between the two commands?*
...
*What happens if you omit the column or the row number? E.g., data_1[2, ]?*
...
Convert the names column from factor to character (save it as a new column)

data_df$names2 <- as.character( ................ )

Check the class of the new and old column

Convert the yob column from factor to numeric (save it as a new column)
...
data_df$yob_R = as.numeric(data_df$yob)
...
We could save it as a separate dataframe

```{r }

data_df2 <- data_df[data_df$yob_R2 < 1980, ]
