...
```{r, echo = FALSE}
# works but saving and reading in as CSV to speed up document creation
crashes <- read.socrata("https://data.cityofchicago.org/resource/85ca-t3if.csv?$where=CRASH_DATE > '2019-01-01T00:00:01'",
...
Get rid of extra variables. Originally had 49, now it will have 29.
```{r}
 crash <- crashes %>% 
  rename_with(str_to_upper) %>% # original code was in all caps and I'm lazy
  select(-c('CRASH_RECORD_ID', 'CRASH_DATE_EST_I', 'TRAFFIC_CONTROL_DEVICE', 'DEVICE_CONDITION', 'LANE_CNT', 'ALIGNMENT', 'ROAD_DEFECT', 'REPORT_TYPE', 'INTERSECTION_RELATED_I', 'PRIVATE_PROPERTY_I','HIT_AND_RUN_I', 'DATE_POLICE_NOTIFIED', 'PHOTOS_TAKEN_I', 'STATEMENTS_TAKEN_I', 'DOORING_I', 'NUM_UNITS', 'INJURIES_INCAPACITATING', 'INJURIES_NON_INCAPACITATING', 'INJURIES_REPORTED_NOT_EVIDENT', 'INJURIES_UNKNOWN')) 



crash$CRASH_DATE <- ymd_hms(crash$CRASH_DATE)  # tells R that date is in month/day/year and hour:minute:second formatting
...
```{r}
# if you read in the data from the original csv and clean the dates, then you will get March 3 2013
#min(crash$CRASH_DATE) # March 3rd, 2013

# if using original csv, then select for all dates after Jan. 1 2019
# this was already done using the API
# crash <- crash %>%
...
# crash <- crash %>% mutate(POSTED_SPEED_LIMIT = case_when(
#   POSTED_SPEED_LIMIT == 1:7 ~ 5, 
#   POSTED_SPEED_LIMIT == 8:12 ~ 10,
#   POSTED_SPEED_LIMIT == 13:17 ~ 15, 
#   POSTED_SPEED_LIMIT == 18:22 ~ 20, 
